-- CRITICAL SECURITY FIXES: Financial Data Protection (Fixed)
...
-- 2. Add PIX key masking function for display
...
BEGIN
  ...
  IF pix_key IS NULL OR length(pix_key) <= 6 THEN
    RETURN '***';
  END IF;
...
-- 3. Add PIX key validation function
...
-- 4. Enhanced withdrawal request validation trigger
...
-- 5. Enhanced bonus claims validation trigger
...
-- 7. Create secure financial data access logging function
CREATE OR REPLACE FUNCTION public.log_admin_financial_access(accessed_table text, accessed_user_id uuid DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  -- Log when admins access sensitive financial data
